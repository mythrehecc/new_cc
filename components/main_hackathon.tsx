...
export default function HackathonPage() {
  ...
  const highlights = [
    ...
    { icon: BookOpen, title: "Learning Experience", desc: "The event provided an opportunity to learn new tools, frameworks, and practices." },
  ...
  const champions = [
    ...
    { 
      ...
      isWinner: true,
      icon: "🏆",
      topColor: COLORS.grandWinnerYellow, // Gold border
  ...
  return (
    <div style={{ background: COLORS.bgBase, fontFamily: FONT_FAMILY, minHeight: '100vh', color: COLORS.textMain, paddingBottom: '40px', paddingTop: '40px', overflowX: 'hidden' }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');
      `}</style>
      
      {/* 1. HERO SECTION */}
      <section style={{ padding: isMobile ? '40px 24px' : '60px 24px', maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
        <div>
          ...
          <h1 style={{ fontSize: isMobile ? '42px' : '64px', fontWeight: 900, margin: '12px 0', lineHeight: 1.1, letterSpacing: '-0.03em', color: COLORS.textMain }}>
            Crestcode <br /> Hackathon
          </h1>
          <p style={{ fontSize: '16px', color: COLORS.textMuted, lineHeight: 1.6, marginBottom: '32px', maxWidth: '600px', marginInline: 'auto', fontWeight: 500 }}>
          ...
          <button 
            ...
            style={{ background: COLORS.primary, color: COLORS.white, padding: '16px 36px', borderRadius: '10px', fontWeight: 800, border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(37, 99, 235, 0.3)' }}
          >
            Explore the Event
          </button>
        </div>
      ...
      {/* 5. INNOVATION CHAMPIONS (Corrected Glow & Shadow) */}
      <section id="champions" style={{ padding: '80px 24px', maxWidth: '1100px', margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ color: COLORS.textMuted, marginBottom: '60px', fontSize: '18px' }}>Celebrating the exceptional projects from our latest edition.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1.2fr 1fr', gap: isMobile ? '30px' : '24px', alignItems: 'center' }}>
          {champions.map((c, i) => (
            <div key={i} style={{ 
              ...
              borderRadius: '24px', 
              border: c.isWinner ? 'none' : `1px solid ${COLORS.border}`,
              borderTop: `6px solid ${c.topColor}`, // Solid colored top border wrapper
              boxShadow: c.isWinner ? '0 30px 60px rgba(0,0,0,0.3)' : '0 20px 50px rgba(0,0,0,0.1)', 
              position: 'relative',
              textAlign: 'center',
              overflow: 'hidden', // Contains the watermark and top border rounding
              ...
              {/* EXACT GLOW AND WATERMARK FOR GRAND WINNER */}
              {c.isWinner && (
                <>
                  <div style={{ position: 'absolute', top: '-10px', right: '-20px', opacity: 0.05, fontSize: '180px', transform: 'rotate(15deg)', pointerEvents: 'none', zIndex: 0 }}>
                    🏆
                  </div>
                  <div style={{ 
                    position: 'absolute', 
                    ...
                    width: '180px', 
                    height: '180px', 
                    background: 'radial-gradient(circle, rgba(250, 204, 21, 0.20) 0%, transparent 60%)', 
                    pointerEvents: 'none', 
                    zIndex: 0 
                  }}></div>
                </>
              ...
              <div style={{ 
                  ...
                  position: 'relative',
                  zIndex: 2,
                  filter: c.isWinner ? 'drop-shadow(10px 10px 8px rgba(0,0,0,0.5))' : 'none'
              }}>
                  {c.icon}
              </div>
              
              <h3 style={{ fontSize: '24px', fontWeight: 900, marginBottom: '8px', position: 'relative', zIndex: 2, whiteSpace: 'pre-line', lineHeight: 1.2 }}>
                  {c.team}
              </h3>
              
              <div style={{ fontSize: '15px', fontWeight: 800, color: c.rankColor, textTransform: c.isWinner ? 'uppercase' : 'none', letterSpacing: c.isWinner ? '0.05em' : 'normal', marginBottom: '24px', position: 'relative', zIndex: 2 }}>
                  {c.place}
              </div>
              
              <p style={{ fontSize: '14px', opacity: c.isWinner ? 0.8 : 1, color: c.isWinner ? '#CBD5E1' : COLORS.textMuted, lineHeight: 1.6, fontWeight: 500, position: 'relative', zIndex: 2, maxWidth: '250px', margin: '0 auto' }}>
                  {c.project}
              </p>
            </div>
          ))}
